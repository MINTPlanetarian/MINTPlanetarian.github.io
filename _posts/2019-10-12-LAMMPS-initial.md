...
显然我们最常用的是atomic，按照使用举例，我们并不需要定义什么参数，参见下图：
...
x,y,z可取`p`/`s`/`f`/`m`中的一个字母或两个字母的组合。
...
dimension命令用来定义模拟的维度。

N可取2或3，表示模拟系统的维度为2或者3。
...
除Boltzmann constant以外，其他参数都与模拟原子本身有关，可以通过查找文献或拟合的方式来确定（至于怎么拟合：作势能曲线图，利用最小二乘拟合）

`lj`类型转换关系见下图：![lj](https://s2.ax1x.com/2019/10/13/uj054K.png)

**2、real类型**

real类型有单位且对参数不做定义和修改。*特别注意：距离单位为埃，能量单位为千卡。*

![real](https://s2.ax1x.com/2019/10/13/ujB9gg.png)

**3、metal类型**

metal类型多适用于金属（我猜的）。*特别注意：能量单位为电子伏特。*
